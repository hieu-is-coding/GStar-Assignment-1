...
def weighted_row_sum_kernel(
    ...
    #  tl.static_print("row_idx, row_start, output_ptr: ", row_idx, row_start, output_ptr)
    ...
    for col_block in range(0, tl.cdiv(N_COLS, BLOCK_SIZE)):
        ...
        #  tl.static_print("col_offsets, mask: ", col_offsets, mask)
        ...
        x_chunk = tl.load(row_start + col_offsets, mask=mask, other=0.0)
        w_chunk = tl.load(W_ptr + col_offsets, mask=mask, other=0.0)
        #  tl.static_print("x_chunk, w_chunk: ", x_chunk, w_chunk)
    ...
    # Thank for the hints, I barely need to review the triton documentation (for now :D).
